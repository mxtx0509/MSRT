...
from pygcn.models_all import GCN
...
def get_loader(adj_dir, feature_dir,txt_dir,SIZE):
    train_list = '/export/home/zm/dataset/ViSR/ViSR_v1.0/list/train.txt'
    test_list = '/export/home/zm/dataset/ViSR/ViSR_v1.0/list/test.txt'

    train_set = SRDataset(adj_dir,feature_dir=feature_dir,txt_dir=txt_dir, file_list = train_list,adj_size=SIZE,graph_mode=args.graph_mode,Threshold=args.threshold,is_train=True)
    test_set = SRDataset(adj_dir, feature_dir=feature_dir,txt_dir=txt_dir, file_list = test_list,adj_size=SIZE,graph_mode=args.graph_mode,Threshold=args.threshold,is_train=False)
...
def main():
    ...
    if args.optim == 'adam':
        ...
        optimizer = optim.SGD(model.parameters(),weight_decay=0.01,lr=args.lr,momentum=0.9)
    else:
        print ('optim input wrong')
...
def validate(val_loader, model, criterion,epoch):
    ...
    for i, (adj_tensor, feature_tensor,target) in enumerate(val_loader):
        ...
        if args.test_mode ==True:
            ep = args.weights.split('_')[0]
            output_ = output.data.cpu().numpy()
            path_result = './result/%s/'%args.graph_mode
            print (path_result)
            if not os.path.exists(path_result):
                os.makedirs(path_result)
            np.savetxt('%s/%s_%s.txt'%(path_result,ep,args.graph_mode),output_)
        ...
        if i % args.print_freq == 0: # print_freq
            print(('Test: [{0}/{1}]\t'
                  'Time {batch_time.val:.3f} ({batch_time.avg:.3f})\t'
                  'Loss {loss.val:.4f} ({loss.avg:.4f})\t'
                  'Prec@1 {top1.val:.3f} ({top1.avg:.3f})\t'
